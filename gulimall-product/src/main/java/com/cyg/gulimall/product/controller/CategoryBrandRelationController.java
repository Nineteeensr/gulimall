...
import com.cyg.gulimall.product.vo.BrandsVo;
...
public class CategoryBrandRelationController {
    ...
    public R relationBrandsList(@RequestParam(value = "catId", required = true) Long catId) {
        ...
        List<BrandsVo> brandsVos = brandList.stream().map(item -> {
            BrandsVo brandsVo = new BrandsVo();
            brandsVo.setBrandId(item.getBrandId());
            brandsVo.setBrandName(item.getName());
            return brandsVo;
        }).collect(Collectors.toList());
        return R.ok().put("data", brandsVos);
    }
